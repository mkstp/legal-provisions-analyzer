...
dbtitle = 'ALT_WBprovisionsdb.csv'
...
for child in soup.h1.next_elements:
    ...
    if child.name == 'li':
        ...
        if 'start' in child.parent.attrs and startflag == 0:
            intIndex = int(child.parent.attrs['start']) - 1
            startflag = 1
        ...
        if enumType == 'lst-spcd' and prevEnumType == 'lst-spcd':
            ...
            alphaIndex = 0
            data.append([str(provNum) + '(' + str(intIndex) + ')' + '(' + alpha[alphaIndex] + ')', part, section, child.get_text()])
            ...
            romanIndex = 0
            data.append([str(provNum) + '(' + str(intIndex) + ')' + '(' + alpha[alphaIndex] + ')' + '(' + roman[romanIndex] + ')', part, section, child.get_text()])
